...
public enum Wrapper {
    BOOLEAN(Boolean.class, "Boolean", boolean.class, "boolean", 'Z', Format.unsigned(1)),
    BYTE(Byte.class, "Byte", byte.class, "byte", 'B', Format.signed(8)),
    SHORT(Short.class, "Short", short.class, "short", 'S', Format.signed(16)),
    CHAR(Character.class, "Character", char.class, "char", 'C', Format.unsigned(16)),
    INT(Integer.class, "Integer", int.class, "int", 'I', Format.signed(32)),
    LONG(Long.class, "Long", long.class, "long", 'J', Format.signed(64)),
    FLOAT(Float.class, "Float", float.class, "float", 'F', Format.floating(32)),
    DOUBLE(Double.class, "Double", double.class, "double", 'D', Format.floating(64)),
    OBJECT(Object.class, "Object", Object.class, "Object", 'L', Format.other(1)),
    // VOID must be the last type, since it is "assignable" from any other type:
    VOID(Void.class, "Void", void.class, "void", 'V', Format.other(0));
    ...
    Wrapper(Class<?> wtype, String wtypeName, Class<?> ptype, String ptypeName, char tchar, int format) {
    ...
    public static Wrapper forWrapperType(Class<?> type) {
        ...
        for (Wrapper x : values())
            if (x.wrapperType == type)
                throw shouldNotReachHere(); // redo hash function
    ...
    private static int hashWrap(Class<?> x) {
        String xn = x.getName();
        final int offset = "java.lang.".length();
    ...
    public Object wrap(Object x) {
        ...
        return switch (basicTypeChar) {
            ...
            default -> {
                ...
                yield switch (basicTypeChar) {
                    case 'I' -> xn.intValue();
                    case 'J' -> xn.longValue();
                    case 'F' -> xn.floatValue();
                    case 'D' -> xn.doubleValue();
                    case 'S' -> (short) xn.intValue();
                    case 'B' -> (byte) xn.intValue();
                    case 'C' -> (char) xn.intValue();
                    case 'Z' -> boolValue(xn.byteValue());
