...
public interface SegmentAllocator {
    ...
    default MemorySegment allocateFrom(String str, Charset charset) {
        ...
        int length;
        if (_StringSupport.bytesCompatible(str, charset, 0, str.length())) {
            length = str.length();
            segment = allocateNoInit((long) length + termCharSize);
            _StringSupport.copyToSegmentRaw(str, segment, 0, 0, str.length());
        } else {
    ...
    default MemorySegment allocateFrom(String str, Charset charset, int srcIndex, int numChars) {
        ...
        MemorySegment segment;
        if (_StringSupport.bytesCompatible(str, charset, srcIndex, numChars)) {
            segment = allocateNoInit(numChars);
            _StringSupport.copyToSegmentRaw(str, segment, 0, srcIndex, numChars);
        } else {
